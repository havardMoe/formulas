...
def load_rankings(
    ...
    with open(filename, mode='r') as f:
        ...
        for row in csv_reader:
            try:
                q_id, d_id = row
                results[q_id].append(d_id)
            except ValueError:
                pass  # ignore rows with missing or corrupted data
    return results
...
def get_precision(
    ...
    Returns:
        P@K (float).
    """
    rankings = set(system_ranking[:k])
    return len(rankings.intersection(ground_truth)) / k
...
def get_average_precision(
    ...
    num_correct_docs = 0
    total_precision = 0
    for i, doc in enumerate(system_ranking, start=1):
        if doc in ground_truth:
            num_correct_docs += 1
            total_precision += num_correct_docs / i
...
def get_mean_eval_measure(
    ...
    # Only care about queries which are both 
    # present in the system ranking and ground truth keys
    queries = set(system_rankings.keys()).intersection(
        set(ground_truths.keys())
    )

    summed_metrics = 0
    for query in queries:
        summed_metrics += eval_function(system_rankings[query], 
                                        ground_truths[query])

    return summed_metrics / len(queries)
